...
public class Game {
    ...
    private List<CardSeries> seriesListInTable = new ArrayList<>();
    ...
    public List<CardSeries> getSeriesListInTable(){
    ...
    public void addInSeries(CardSeries series, Card card){
    ...
    public boolean isCardTooWeak(Card card){
        ...
        for (CardSeries series : seriesListInTable)
        {
    ...
    public CardSeries getTheSeriesWithSmallestDifference(Card card){
        List<Integer> diffList = new ArrayList<>();
        List<Integer> positiveList = new ArrayList<>();
        for (CardSeries series : this.seriesListInTable)
        {
    ...
    public void initSeries(int position){
        ...
        CardSeries series = new CardSeries(position, cardList);
    ...
    public void processForCardTooWeak(int seriesIndex, Player currentPlayer, Card playerCard){
        CardSeries takenSeries = seriesListInTable.get(seriesIndex - 1);
        seriesListInTable.set(seriesIndex - 1, CardSeries.newSeries(takenSeries.getPosition(), playerCard));
        currentPlayer.removeCard(playerCard);
        List<Card> pack = currentPlayer.getPack().getCards();
        pack.addAll(takenSeries.getCardsInTable());
        currentPlayer.setPack(new RetrievedSeries(pack));
    }

    public void normalProcess(Player currentPlayer, CardSeries chosenSeries, Card playerCard){
        addInSeries(chosenSeries, playerCard);
        currentPlayer.removeCard(playerCard);
    }

    public void processForFullSeries(Player currentPlayer, CardSeries chosenSeries, Card playerCard){
        CardSeries newSeries = CardSeries.newSeries(chosenSeries.getPosition(), playerCard);
